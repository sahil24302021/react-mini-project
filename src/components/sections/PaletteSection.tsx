...
import { isColorLight, exportCss, exportJson, exportTailwind, exportScss, exportSwiftUI, exportCompose, simulateCVD, type CVDType } from '@/utils/colorUtils'
    ...
    <section className="max-w-6xl mx-auto px-5 md:px-8 py-20 flex flex-col gap-16">
      ...
      >
        {color.palette.map((swatch) => {
          const displayHex = cvdType === 'none' ? swatch.hex : simulateCVD(swatch.hex, cvdType as CVDType)
      ...
      {showExport && (
        ...
        >
          ...
          <div className="flex border-b overflow-x-auto scrollbar-hide" style={{ borderColor: 'var(--border)' }}>
            {(['css', 'scss', 'tailwind', 'swiftui', 'compose', 'json'] as ExportFormat[]).map(fmt => (
              <button
                key={fmt}
